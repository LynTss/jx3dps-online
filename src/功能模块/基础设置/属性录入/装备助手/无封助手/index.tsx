...
const 识别装备对比: React.FC<ModalProps> = () => {
        ...
        if (item.装备部位 !== 对应装备数据?.装备部位) {
          ...
          return {
            ...
            镶嵌孔数组: 对应装备数据?.镶嵌孔数组?.map((a, index) => {
              return {
                ...a,
                镶嵌宝石等级: item?.镶嵌孔数组?.[index]?.镶嵌宝石等级 || 8,
              }
            }),
            附魔: item?.附魔,
          } as any
  ...
  const 组合计算结果 = useMemo(() => {
    ...
    组合计算装备.forEach((装备) => {
      ...
      const 装备最大精炼等级 = 获取最大精炼等级(对应装备数据)
      新装备列表 = 新装备列表.map((item) => {
        if (item.装备部位 !== 装备?.装备部位) {
          return item
        } else {
          return {
            id: 对应装备数据.id,
            ...
            镶嵌孔数组: 对应装备数据?.镶嵌孔数组?.map((a, index) => {
              return {
                ...a,
                镶嵌宝石等级: 对应装备数据?.镶嵌孔数组?.[index]?.镶嵌宝石等级 || 8,
              }
            }),
            附魔: 对应装备数据?.附魔,
          } as any
          ...
          {开启组合计算 ? (
            <div className='wufeng-group-dps-res'>
              ...
              <div className='wufeng-group-dps-res-equips'>
                ...
                {组合计算装备?.map((item) => {
                  const 获取装备信息 =
                    装备数据?.[item?.装备部位]?.find((item) => item?.id === item?.id) || ({} as any)

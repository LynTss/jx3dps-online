...
export const 获取心法循环 = (params) => {
  const { 心法 = '山海心诀'} = params || {}
  const 当前数据 = 获取当前数据(心法)
  const 全部循环信息 = 当前数据.计算循环 || []
  const 该心法支持循环数据 = 全部循环信息?.map(item => {
    return {
      名称: item?.名称, // 唯一ID
      类型: item?.类型,
      标记: item?.标记,
...
export const 计算秒伤 = (params) => {
  const { 心法 = '山海心诀', 奇穴 = [], 装备列表 = [], 装备增益 = {}, 五彩石 = { 属性数组: [] }, 循环名称:入参循环名称 = "" } = params || {}
  ...
  if (入参循环名称) {
    // 判断传入的循环名称该心法是否支持
    const 存在该循环 = 全部循环信息?.find(item => item?.名称 === 入参循环名称)
    if (存在该循环) {
      循环名称 = 入参循环名称
    } else {
      循环名称 = 获取对应的循环名称(心法, 奇穴, 循环类型)
    }
  } else {
    循环名称 = 获取对应的循环名称(心法, 奇穴, 循环类型)
  }
